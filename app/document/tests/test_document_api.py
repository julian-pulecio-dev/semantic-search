...
class DocumentViewsTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.doc1 = Document.objects.create(
            status=Document.Status.READY, user=self.regular_user, s3_key="key1"
        )
        self.doc2 = Document.objects.create(
            status=Document.Status.PENDING, user=self.other_user, s3_key="key2"
        )

    def test_list_all_documents_admin_only(self):
        """Validate that only admin users can access the list of all documents."""
    ...
    def test_delete_document_and_s3_call(self, mock_delete):
        """Validate that deleting a document also triggers the S3 file deletion."""
